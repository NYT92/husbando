...
async function submitForm(event) {
  ...
  if (formData.source.length > 0) {
    if (!formData.source.includes("https://")) {
      modalTitle.value = "Error";
      ...
      await $fetch("/api/upload", {
        ...
        .then((res) => {
          if (res.success) {
            ...
            imgDataResp.value = res.data;
          } else {
            modalTitle.value = "Error";
            modalContent.value = "Something went wrong";
            openModal.value = true;
            progressbar.value = 0;
            loadingWhenUpload.value = false;
          }
...
<template>
  ...
  <UContainer>
    ...
    <div class="py-6">
      <h1 class="font-bold text-2xl">Upload your husband picture {{ `:)` }}</h1>
      <UAlert class="my-2" title="Upload Guidelines">
        <template #description>
          <p>
            By using our services, you must comply with the guidelines such as:
          </p>
          <ul
          ...
          >
            ...
            <li>
              Do not upload R-18 shota, illegal IRL media, meme, hate speech and
              gore.
            </li>
            <li>
    ...
    <UForm class="pb-10 gap-5 flex flex-col" @submit="submitForm">
      <UProgress id="progress" v-if="loadingWhenUpload" :value="progressbar" />
      ...
      <UFormGroup label="NSFW" name="isNsfw">
        ...
        />
        <br />
        <span v-if="formData.isNsfw">Make sure the image contained nudity</span>
      </UFormGroup>
      ...
      <UFormGroup label="List publicly?" name="isPublic">
        ...
        <span v-else-if="!formData.isPublic">
          ⚠ This will not be listed on Homepage list, but still can be access
          from the page & API.
        </span>
      </UFormGroup>
  ...
  <section v-else>
    <UContainer class="my-5">
      <h1 class="font-bold text-2xl text-center">Image has been uplodaded!</h1>
      <p class="text-center">
